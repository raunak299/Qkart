...
function Cart() {
    ...
    const { cartProductList, replaceCartProductList, changeQuantityHandler, deleteCartHandler } = useContext(CartContext)
    ...
    useEffect(() => {
        const finalTotalPrice = cartProductList.reduce((tempTotalPrice, product) => {
    ...
    return (
        <React.Fragment>
            ...
            <div className={styles['cart-page']}>
                {(!cartProductList || cartProductList.length === 0) && <h1 className={styles['no-product-msg']}>No item in cart!!</h1>}
                {cartProductList?.length > 0 && <div className={styles['cart-section']}>
                        ...
                        {cartProductList.map((product) => (
                            <div className={styles['cart-product']} key={`${product['_id']}&${product['sizeToBuy']}`}>
                                ...
                                <div className={styles['product-detail']}>
                                    ...
                                    <div>Size: <span>{product.sizeToBuy}</span></div>
                                    <div className={styles['add-to-cart-sec']}>
                                        <div className={styles['change-quantity']}>
                                            <button value={product['_id']} onClick={changeqtyHandler} >-</button>
                                            <div>{product.qty}</div>
                                            <button value={product['_id']} onClick={changeqtyHandler}>+</button>
                                        </div>
                                    </div>
                    ...
                    <div className={styles['cart-summary']}>
                        ...
                        {cartProductList.map((product) => (
                            <div className={styles['product-in-cart']} key={`${product['_id']}&${product['sizeToBuy']}`}>
